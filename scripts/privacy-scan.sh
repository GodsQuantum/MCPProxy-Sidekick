...
for file in "${files[@]}"; do
  case "$file" in
    scripts/privacy-scan.sh|docs/assets/dashboard.png) continue ;;
  esac
...
emails="$(grep -nE --binary-files=without-match -- "[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}" "${scan_files[@]}" 2>/dev/null || true)"
if [[ -n "$emails" ]]; then
